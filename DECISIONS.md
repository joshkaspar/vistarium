...
Outcome: `build_site.py`'s `is_open_license` import stays (used to compute `license_category`), the exclusion branch is gone. 2 tests in `test_build_site.py` updated from exclusion-assertions to tagging-assertions; the pipeline license-skip test replaced with one confirming restricted candidates flow through normally. Not yet rebuilt.
