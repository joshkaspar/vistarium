...
## 2026-09-07: "photo contest" album-exclude reverted -- a fifth rejected heuristic, same failure shape as the other four

Context: the "photo contest" exclude terms added to `album_keywords.json` the day before (see the entry above) turned out to have the same problem already found and rejected four times over for other signals (credit-line format, GrantingRights level, album-level trust, and -- per that entry's own framing -- these were meant to be the fix, not another instance of the pattern). Real counter-example: "Lower Hamilton Lake" (Sequoia & Kings Canyon, `dfc45e04-1dd8-b71b-0b21-3d2b8180dd09`) is from a "2016 Facebook Photo Contest Winners" album, credited to an individual photographer, and would have matched the exclude term -- but its `Constraints Information` is a clean bare `Public domain`, no red flags anywhere, and manual review had already flagged it as one of the best images in the whole dataset. "Photo contest" correlates with copyrighted submissions (it held for the Bryce Canyon and NERI/Comerci examples that motivated adding it) but doesn't determine it -- some parks' contest terms require a public-domain release as a condition of entry, others don't, and there's no way to tell which from the album name alone.

Corrected a factual premise while investigating this: the request described a per-item title/Keywords/Description check as also having been added alongside the album-level one. It hadn't been -- that gap was found and explicitly deferred (Josh's call: "skip for now") during the 2026-09-06 verification pass in the entry above. Only the album-level `album_keywords.json` terms existed to remove.

Decision: removed `"photo contest"`/`"photography contest"`/`"photo challenge"` from `album_keywords.json`'s exclude list entirely -- not loosened to a soft flag, not scoped to specific parks. `"repeat photography"`/`"repeat photo"` are unaffected and stay -- that exclusion rests on the Denali collection's own explicitly-stated educational-use-only terms (a real, confirmed restriction on that specific collection, found in its `Copyright` field text), not a category-level assumption about contest submissions in general. The two real risks the contest rule was trying to catch are already handled correctly, per-item, without it: a genuinely public-domain contest entry publishes correctly via the existing `Constraints Information` parsing (`is_open_license()`), and a mislabeled entry with a real watermark is exactly what the narrowed `license_confidence` prompt (same day prior, this file) is for.

Confirmed before touching anything: `data/catalog.json`/`checkpoint.jsonl` on wopr were last modified 2026-09-05, before `album_keywords.json` was even edited to add the terms (2026-09-06) -- no scrape has run with this rule active, so there was nothing to recover. `dfc45e04...` is confirmed present in the current `docs/data.json`.

Outcome: resolved. `album_keywords.json`'s `_comment` updated to record the addition-then-removal and the specific counter-example, as a marker against re-adding this or a similar category-level heuristic later.

## 2026-09-07: review-tool thumbnails were cropping away the exact defects they exist to catch

Context: Josh found a promotional graphic (`e86f3f13...`, Shenandoah "Sunset" -- a real photo with a John Muir quote banner burned in at top and a "Find your park" caption bar at bottom) that had been swept into the `confirmed_ids.json` bulk migration. Asked why it wasn't caught during review: "I didn't see it because the tool crops the images."

Root cause confirmed in both `license_review_server.py` and `dedup_review_server.py`: `.member img { height: <fixed>px; object-fit: cover; }` -- `cover` inside a fixed-height box crops a portrait (9:16) image's top and bottom to fill a landscape-ish card, and that's exactly where overlay text/watermarks/caption bars tend to sit. The underlying `docs/thumbs/*.webp` file does show the full frame (already verified by hand); the review tool's own CSS was re-cropping it on top of that, specifically hiding the content a human reviewer most needs to see.

Decision: both tools switched to `object-fit: contain` with a `max-height` cap and a black letterbox background, so the whole frame is always visible regardless of aspect ratio, at some cost to visual density/uniformity in the grid. This is a correctness-over-polish tradeoff appropriate for a tool whose entire purpose is spotting defects -- a review tool that hides the defect is worse than no review tool.

Same-park sweep once the fix was in: checked all remaining Shenandoah `flagged_for_review` records by eye and found 3 more instances of the same "Find Your Park" marketing series (`e8823679...` "Re-creation", `e886b816...` "Mountains", `e873a926...` "Skyline Drive" -- the last with a full 2016 NPS Centennial logo lockup), plus confirmed 5 others in the same set are genuine photos correctly flagged only for a person in frame, not overlays. All 4 promotional-graphic instances hidden.

Also cleaned up two Yellowstone duplicate clusters found live while investigating a user report of still-live duplicates from the 2026-09-06 `find_duplicates.py` run: that run's timestamp clusters were surfaced for review but never actually reviewed. The "Snow dunes, Hayden Valley" cluster (reported directly) turned out to contain a second, separate duplicate trio ("Winter, Hayden Valley") riding along in the same 7-photo timestamp window, plus one genuinely distinct black-and-white edit ("Steam from Mud Volcano") correctly left alone. A second cluster ("Electric Peak", 3 copies) was also still fully live. 6 duplicate records hidden across both clusters, keeping the highest-`aesthetic_score` copy from each -- in every case matching `find_duplicates.py`'s own `suggested_keep`.

A third overlay variant also turned up the same day: `9116671b...` (Acadia, "215 Sounds of the Sea") is a full interpretive/wayside-exhibit panel design -- title, body text, an illustrated cave diagram, a "Safety Tips" box -- composited over a real photo. Unlike the other two variants, this one was never `flagged_for_review` at all (`license_confidence: confirmed`); it's a `primary_subject` miscall, not a license-flag miss -- schema.json's own "document" category exists exactly for this ("a photographed newspaper page, museum placard, interpretive sign, map, or screenshot"), but the model classified it `landscape`, probably because the dominant visual content (the background photo) reads as scenic despite the overlay. A site-wide sweep for similarly-numbered titles found no siblings, but that was a narrow, title-based check -- a `primary_subject` miscall like this could exist elsewhere with no comparable tell. Hidden.

Outcome: resolved. Both review tools fixed and redeployed; `license_review_server.py` restarted. `hidden_ids.json` grew by 11 records this pass (4 promotional graphics, 1 wayside-panel graphic, 6 duplicates) -- not yet rebuilt. The larger, unresolved points stay on `ROADMAP.md`: `find_duplicates.py`'s timestamp clusters (367 as of the last run) still need an actual review pass, not just detection; the 351-record `confirmed_ids.json` migration was a status-quo carry-forward, not a review, so more of the same overlay/graphic problem likely remains in it undiscovered; and the wayside-panel case specifically suggests `primary_subject` miscalls on non-photograph-shaped content may be a broader gap than just the license-flag path.

## 2026-09-07/08: `fire_smoke_category` field -- fire/smoke/volcanic content, keep-worthy landscape vs. incident documentation

Context: Yellowstone's `ee9c9992...` ("Druid Complex" fire) investigation found its only `RelatedCollections` tag ("NPS.gov Collection") isn't a real album -- confirmed via `list_park_albums('YELL')` (266 real albums, no match) -- meaning most fire/smoke/volcanic content had never passed through any album-level triage at all, keyword or otherwise. Josh wrote a full policy spec (dated 2026-09-07) framed around one "core test": is fire/smoke/volcanic activity the documented EVENT a photo exists to record, or incidental atmosphere within an otherwise landscape-forward composition? -- with 5 subcategories and 11 cited real examples with keep/reject verdicts.

Design choice, via `AskUserQuestion`: an enum field (`fire_smoke_category`, 7 values), not a boolean -- so a wrong call stays inspectable/correctable per-category rather than collapsing to an opaque flag. Same validate-before-rollout discipline as `minor_face_present` (see the 2026-09-06 entry above): tested against the 11 hand-labeled examples before running at any scale.

First pass: 7/11 exact-label matches. But the field's only real consequence is the keep/reject bucket, not the exact label -- re-scored on that basis, 10/11 correct, with the one true miss (`54b499f2...`, "Old Rag Fire") being a boundary case both times. Two of the four label-level misses were aerial shots the prompt hadn't yet distinguished from ground-level ones. Josh's fix, via `AskUserQuestion`: for aerial shots, exclude ones focused on smoke/fire but keep ones focused on eruptions; for ground-level shots, exclude only if the compositional focus is smoke/fire/response-infrastructure, keep if the focus is the landscape. Redeployed and re-tested: still 7/11 exact, different failure set (fixed the aerial case, regressed a label-only case), same 10/11 practical accuracy. Josh explicitly deprioritized the remaining geyser/volcano confusion risk: "the parks that it can happen on are narrow... we aren't going to see geysers in Everglades and we aren't going to see Volcanoes in Smoky Mountains."

Full batch: ran the validated prompt against all 207 currently-published records carrying a fire/smoke-adjacent keyword (`scripts/_run_fire_batch.py`, one-off, not committed) -- 113 keep / 94 reject, 0 errors. Results loaded into a new `scripts/fire_review_server.py` (port 5153, same pattern as the other two review tools, `object-fit: contain` from the start this time) pre-seeded with the model's keep/reject call plus its `fire_smoke_category`/`fire_smoke_evidence` per card, for a final human pass -- not treated as good enough to auto-publish/hide unreviewed, consistent with every other model-driven filter on this project.

Josh's review, once complete (207/207 decided, 117 keep / 90 hide): "Overall, successful addition... a little aggressive with ground level 'aftermath' categorization... 2 or 3 were miscategorizations, but the rest were because I thought the composition was worth keeping." Read plainly: the `burn_aftermath_dominant`-default-over-`burn_aftermath_resilience` stance (see prompt text) over-triggers somewhat on genuinely keep-worthy ground-level aftermath compositions, but this is real aesthetic/compositional judgment the model can't fully replicate, not a comprehension bug -- and Josh was explicit he "wouldn't change anything about the way we prompted it." No prompt change made for this feedback; corrected entirely through the human review pass instead, same as `license_confidence`'s `flagged_for_review` gate handles its own false-positive rate.

Second decision, prompted by Josh asking whether "the general principal" behind this ("documented event vs. incidental atmosphere") could be added to the prompt as it judges images generally, not just for this field: yes -- added a short scoped paragraph to `model_client.py`'s shared `PROMPT` preamble (above the per-field list, not inside `fire_smoke_category`'s own text) generalizing the test to any field asking whether something is a photo's real subject vs. incidental (fire/smoke, a structure, an activity, wildlife, a person). This is the same test already proven twice before under different names -- the (reverted) `STRUCTURE_ALLOWED_PARKS` discussion and `primary_subject`'s landscape-vs-detail distinction both turn on the same compositional-dominance judgment. Re-ran the 11-example fire/smoke validation set after deploying the preamble change specifically to confirm it didn't regress the field it was extracted from, since a shared-preamble edit risks bleeding into every other field's judgment, not just the one it was designed around.

`hidden_ids.json` grew by 66 new records from this review (24 of the 90 hides were already hidden from other passes), bringing the total to 591 -- not yet rebuilt/republished.

Outcome: resolved and deployed. `fire_smoke_category`/`fire_smoke_evidence` are future-scrapes-only fields, same as `content_visible`/`minor_face_present` -- no backfill onto the existing corpus; the 207-record backlog was triaged separately via the dedicated review tool and manual `hidden_ids.json` edits, not an automated `build_site.py` gate on this field. No automated `fire_smoke_category` exclusion has been wired into `build_site.py`'s `eligible` filter -- unlike `minor_face_present`, this field's reject categories aren't hard-gated at publish time for any *future* scrape either; revisit if that gap matters once new curated scrapes start picking up fire/smoke content again.

## 2026-09-08: `find_duplicates.py`'s 367 timestamp clusters fully reviewed; a cheap-VLM triage assist raised and deferred

Context: the 2026-09-06 `find_duplicates.py` run left 367 EXIF-timestamp clusters flagged for manual review (27 exact-hash clusters auto-hide and never needed review). Repeated smaller passes through this backlog kept surfacing unrelated problems the same way the license/overlay-graphic review did (see the 2026-09-07 entry above) -- looking closely at every image in a cluster to judge "same shot or not" doubles as a free audit of everything else in frame. Josh worked through the full 367 by hand in `dedup_review_server.py` (`http://192.168.68.136:5151/`) and confirmed completion.

Mid-review, Josh asked whether a cheap cloud VLM (e.g. Haiku) could have done the cluster-level keep/hide call. Assessment given and agreed: yes, in principle -- "are these the same shot" and "which one's strongest" is a narrower, more tractable judgment than `fire_smoke_category`'s compositional-dominance test, and would fit the same validate-before-rollout pattern already proven twice this project (hand-label a sample, check practical accuracy, then run at scale with output landing in a review tool rather than auto-applied, to preserve the free side-audit). Josh's response: "changed the scope of the project overall" -- wants to think it over before committing, not a yes or no yet. Logged as a possibility on `ROADMAP.md`, not started.

`hidden_ids.json` grew by 44 records from this specific review pass (folded in and republished same day, see the entry above -- 3,725 -> 3,681 published records).

Outcome: resolved for the current backlog. The 367-cluster review that had been "largely unreviewed" throughout this session's fire/smoke and license work is now closed out. The cheap-VLM-assist idea stays open on `ROADMAP.md`, explicitly framed around future batches (new sources, future `find_duplicates.py` re-runs) rather than this cleared backlog.

## 2026-09-08: YOSE NPGallery portal `Rating` field investigated as a possible quality signal -- turns out to be a per-batch constant, not a per-image score

[data-gathering only, no pipeline/data changes -- see the originating investigation below for why Yosemite is thin in the curated catalog]

Context: Yosemite publishes N=10 in the curated catalog vs. Yellowstone's 456/Denali's 556 -- not an album-triage gap (checked by hand). Yosemite's current staff-credited photography lives on `nps.gov/yose/`'s homepage carousel as individually-embedded CMS assets, not a scrapable index. The real archive is Yosemite's own dedicated NPGallery portal, `npgallery.nps.gov/YOSE/` -- a separate archival digitization project (20,000+ historic images, ~6,500 digitized) distinct from the general npgallery.nps.gov search, confirmed as the park's official public photo resource via Yosemite's own "Photos & Multimedia" page. Quality inside it varies wildly by named "Hierarchy Series" (e.g. "Lantern Slides" = unusably low-res 1930s scans; "Scenics-Lakes" = full-resolution TIFFs up to 28.7 MB) -- the portal's own Hierarchy Series filter, not free-text keyword search (noisier, mixes survey/exhibit-planning photos into genuine scenics), is the right tool for scoping this. Individual `Scenics-*` images still visually read as lower technical quality than Vistarium's modern photography, and neither the VLM (thumbnail-based judgment) nor `aesthetic_score` (CLIP-based, composition/appeal) was ever built to catch "soft/faded old scan" -- this is what originally surfaced the Yosemite Slide Collection out-of-focus problem a few steps into this investigation. Individual asset pages have an inconsistently-populated 1-5 star `Rating` field, hypothesized as a possible per-item technical/visual quality signal worth checking before deciding whether to pursue this portal further.

**Endpoint discovery**: `npgallery.nps.gov/YOSE/SearchResults` looked entirely client-rendered on first inspection, but a plain GET with the right query params (`collection=<name>&controlledkeyword=<Hierarchy Series value>&pagesize=N`, matching the portal's own search form field names -- `select#yoseGroup` name="collection", `select#yoseSeries` name="controlledkeyword") returns the exact same embedded `var search = {...}` JSON payload `nps_client.extract_payload()` already parses for the general npgallery search -- no separate AJAX/JSON API needed. Confirmed against the portal's own dropdown: the full `Scenics-*` Hierarchy Series list has 18 values, not the 14 previously known (`Scenics-Caves`, `Scenics-Other Areas`, `Scenics-Special Effects`, `Scenics-Unidentified` were missing from the earlier partial list). `File Size (bytes)` and `Photographer` are NOT in the search JSON payload -- only on the per-asset `AssetDetail` HTML page, one extra request per asset.

**The `Rating` finding, established before the pilot pull even needed to run at scale**: sorting any named collection by `Rating` descending (the payload's own default `SortTerms`) and checking the top of a large result set shows *zero variance* -- every single record shares one value. Checked directly: `1031 Yosemite Historic Photo Collection` (13,663 records, the parent of all `Scenics-*` series) is uniformly `Rating: 1` at the top of a descending sort; `Yosemite Slide Collection` (6,437 records -- the collection containing the 1952 slide scan whose 2-star rating originally sparked this hypothesis) is uniformly `Rating: 2`, confirmed exhaustively across all 13 pages via pagination, not just the top; `County of Mariposa Collection` is uniformly `Rating: 3`. The one collection that did show real variation, `Yosemite Public Affairs Office Collection` (42 records), turned out to track distinct photo-op *events*, not per-image quality within an event -- every "Congressional Delegation" visit photo scored 3, every "COVID-19 vaccine clinic" photo scored 1, with no variation inside either event's own photo set. Conclusion: `Rating` is set once per submission batch by whoever cataloged/imported it (plausibly reflecting the submitting archivist's own sense of the batch's institutional priority or completeness), not a per-image technical/visual quality judgment -- the exact hypothesis this investigation set out to test.

**`Scenics-Canyons` pilot pull** (`scripts/_investigate_yose_rating.py`, all 75 results + one `AssetDetail` fetch per asset for file size/photographer, respecting the shared `nps_client._http_request()` rate limiter throughout): confirms the finding at the pilot's actual intended scope, not just from spot-checks -- all 75 records: `Rating: 1`, `Constraints Information: Public domain` (uniform, no restricted-license outliers in this sample), file sizes 5.1-40.4 MB (all well above the wallpaper resolution floor -- smallest was 1981x1358, still comfortably over `MIN_WALLPAPER_LONG_EDGE`), photographer credited on 57/75 (18 "Unknown"), 1 placeholder "NA" title. One `Constraints Information`-vs-archivist-note contradiction found, same pattern as the Comerci/Galloway cases from earlier license work: asset `467ecbcb...` ("YOSE RL_01088" / Tenaya Canyon) has `Constraints Information: Public domain` but a cataloger `Comment` reading "Could not find photographer on Google or LOC copyright database" -- not resolved here, just surfaced per the same discipline (trust the `Constraints Information` prefix, but a contradictory note should ride along in `license_evidence` if this source is ever actually onboarded).

Given `Rating` showed zero within-collection variance well before the pilot completed, the planned expansion to the other 17 `Scenics-*` series was paused rather than run -- would have cost hours of rate-limited requests to very likely reconfirm the same null result. `SCENICS_SERIES_FULL` (the complete 18-value list) is left in the script for whenever/if a different question needs it.

Outcome: resolved as originally scoped -- this was diagnostic-only, no pipeline or catalog change, per Josh's explicit instruction. `Rating` is not usable as a quality signal for the reason hypothesized; whether the YOSE portal is worth onboarding as a source at all is still an open, separate question (Josh's to decide, not addressed here) now informed by this data rather than the original screenshot's single data point. Raw pilot data (`/tmp/yose_rating_investigation.json`, 75 records, gitignored/local-only) available for direct visual review of thumbnails per record.
